...
- **รีเซ็ตโควตา**: หลังบ้าน → โค้ช AI → "โควตาวันนี้" รีเซ็ตรายบัญชี / ราย IP / ทุกคน — เก็บเป็น `settings.ai.resets` (`all`, `u:<uuid>`, `ip:<hash>` = เวลา) แล้ว API นับเฉพาะคำตอบหลังเวลานั้น (อ่าน settings แบบ fresh ทุกคำถาม จึงมีผลทันที)
- **ผู้ให้บริการรับ 1 คำขอค้าง/คีย์**: `chat()` เจอ 429 "waiting queue is full" จะรอ 4 วิแล้วยิงใหม่ภายใน 54 วิ (Vercel ให้ 60), คำขอที่มาถึง instance เดียวกันต่อคิวใน `advise.js` (`serialize`), ถ้ายังไม่ว่างตอบ 503 `BUSY` และ `aiClient` ลองใหม่เองอีก 3 ครั้งห่าง 5 วิ; แอดมินไม่ติด burst 6/นาที
