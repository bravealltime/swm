...
3. ถ้ามี "กล่องของผู้ใช้" แนบมา ให้แนะนำจากมอนสเตอร์/รูนที่เขามีจริงเท่านั้น ชื่อผู้เล่นในบรรทัดแรกของกล่องคือตัวผู้ใช้เอง — ถ้าเขาถามถึงชื่อนั้น ให้ตอบจากกล่อง
4. ชื่อที่ไม่ใช่มอนสเตอร์มักเป็นชื่อผู้เล่น ถ้ามี "ข้อมูลผู้เล่น SWRT" แนบมาให้ใช้ ถ้าไม่มีทั้งในกล่องและ SWRT ให้บอกสั้น ๆ ว่าไม่พบผู้เล่น/มอนสเตอร์ชื่อนี้ในระบบ และแนะนำให้นำเข้ากล่องที่หน้า "กล่องของฉัน" หรือค้นหาในหน้าผู้เล่น ห้ามเดาชื่อที่คล้ายกัน
5. จัดรูปแบบให้อ่านง่าย: ขึ้นต้นด้วยคำตอบสั้น 1 บรรทัด แล้วตามด้วยหัวข้อสั้น ๆ + bullet หรือขั้นตอน 1. 2. 3. เมื่อเป็นแผน ยาวไม่เกิน ~220 คำ ไม่ต้องทักทาย`;
...
function chatPrompt({ question, context = {}, history = [] }) {
  ...
  if (Array.isArray(context.players) && context.players.length) parts.push(`ข้อมูลผู้เล่น SWRT (สถิติ RTA สาธารณะ):${NL}${context.players.slice(0, 2).map((p) => `- ${String(p).slice(0, 600)}`).join(NL)}`);
