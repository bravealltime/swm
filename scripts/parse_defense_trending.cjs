...
const file = path.join(__dirname, '..', 'swgt_raw', 'monsterDefenseTrending.html');
...
for (let i = 1; i < trMatches.length; i++) {
  ...
  if (imgMatch && tds.length >= 2) {
    ...
    items.push({
      ...
      pickShare,
      winRate,
      winRateNum: parseFloat(winRate.replace('%', '')) || 50
    });
  }
}

console.log('Total defense trending monsters parsed:', items.length);

const outPath = path.join(__dirname, '..', 'src', 'data', 'monsterDefenseTrending.json');
